...
static RPCGateway rpcGateway("instance-2.c.quick-elixir-137423.internal", 9090);

static int outstanding_writes=0;

static WriteCache writeCache(rpcGateway);
...
//    cout << "write size: "<< size << " data: " << buf << endl;
    ...
    outstanding_writes++;
    WriteCacheEntry entry(fi->fh, path, buf, size, offset, fi);
    writeCache.insert(entry);
...
{
    int res;
    res = rpcGateway.xmp_release(path, fi, outstanding_writes);

    if(res < outstanding_writes) {
        writeCache.writeAll();
    }
...
{
    int res;
    res = rpcGateway.xmp_fsync(path, isdatasync, fi, outstanding_writes);
